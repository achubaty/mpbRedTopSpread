...
objFunInner <- function(
  ...
  # massAttacksRas <- rast(massAttacksStack[[startYears]])
  ...
  #   as.data.table(eval(quotedSpread, envir = list(...)))#)
...
assign("objFunInner", envir = .GlobalEnv, function(
    quotedSpread,
    startYears, endYears,
    massAttacksStart, massAttacksEnd,
    greenTreesYr_t, pixelYr_t,
    massAttacksDT,
    clNumber = NULL,
    p,
    ...,
...
#' Objective function to minimize for mpbRedTopSpread
assign("objFun", envir = .GlobalEnv, function(p, propPineRas,
                   massAttacksStack, massAttacksDT,
                   quotedSpread,
                   omitPastPines, p_sdDist, dispersalKernel,
                   maxDistance, windDirStack, windSpeedStack,
